...
interface Team {
  ...
  leaderAffiliation: string | null
  members: string | null
  researchTopic: string | null
  users: TeamUser[]
}

export default function TeamClient({ initialTeam }: { initialTeam: Team | null }) {
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState('')
  const [form, setForm] = useState({
    leaderName: initialTeam?.leaderName || '',
    leaderAffiliation: initialTeam?.leaderAffiliation || '',
    members: initialTeam?.members || '',
    researchTopic: initialTeam?.researchTopic || ''
  })
