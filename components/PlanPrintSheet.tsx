...
// A4 예산 사용 계획서 시트 — /print 페이지와 관리자 인쇄 모달에서 공유
export default function PlanPrintSheet({ plan, userName, teamNumber, signature }: Props) {
  // 사용자명 포맷 '홍길동/학생/교육학과' → [이름, 신분, 학과]
  const nameParts = (userName ?? '').split('/').map((part) => part.trim())
  const displayName = nameParts[0] ?? ''
  const affiliation = nameParts[2] ?? (teamNumber ? `${teamNumber}팀` : '')
